...
try:
    if os.path.exists('NotoEmoji-Regular.ttf'):
...
class PushNotification:
    ...
    def send(user_id, title, message):
        ...
        try:
            ...
            _notif.notify(title=title, message=message, timeout=6)
        except Exception:
            pass

_mensajes_notificados = set()

def _revisar_mensajes_nuevos(dt=None):
    """Revisa periódicamente si llegaron mensajes de chat nuevos dirigidos
    al usuario actual (de un director a un líder, o entre líderes) y muestra
    una notificación local si la app está abierta."""
    ...
    def on_data(success, data):
        ...
        for mid, m in data.items():
            if mid in _mensajes_notificados:
                continue
            if m.get('destinatario_id') == app.user_id and m.get('remitente_id') != app.user_id:
                _mensajes_notificados.add(mid)
                PushNotification.send(
                    app.user_id,
                    f"Nuevo mensaje de {m.get('remitente_nombre', 'un compañero')}",
                    m.get('mensaje', '')
                )

    firebase_get("chat", on_complete=on_data)
...
# ========================
# 📥 GUARDAR ARCHIVOS EN LA CARPETA DE DESCARGAS DEL CELULAR
# ========================
def _guardar_archivo_publico(nombre_archivo, contenido_texto, mime='text/csv', encoding='utf-8'):
    """Guarda el archivo directamente en la carpeta pública de Descargas del
    celular (funciona en Android 10 y superior, sin pedir permisos, usando
    MediaStore). Si no es posible (por ejemplo probando en computadora),
    lo guarda en la carpeta privada de la app como respaldo, mostrando
    el motivo técnico del fallo para poder diagnosticarlo."""
    datos_bytes = contenido_texto.encode(encoding, errors='replace')
    try:
        ...
        values.put(MediaStore.MediaColumns.DISPLAY_NAME, nombre_archivo)
        ...
        if uri is None:
            raise Exception("resolver.insert() devolvió None: no se pudo crear el registro en MediaStore")

        stream = resolver.openOutputStream(uri)
        stream.write(datos_bytes)
        stream.flush()
        stream.close()

        try:
            values2 = ContentValues()
            values2.put(MediaStore.MediaColumns.IS_PENDING, 0)
            resolver.update(uri, values2, None, None)
        except Exception:
            pass

        return True, f"Descargas/{nombre_archivo}"
    except Exception as e:
        try:
...
def guardar_csv_publico(filename, headers, filas):
    """Genera un CSV en memoria y lo guarda en Descargas."""
    buffer = io.StringIO()
    ...
    exito, ubicacion = _guardar_archivo_publico(filename, contenido, mime='text/csv')
    if exito:
        mostrar_popup_exito(f"✅ Archivo guardado en:\n{ubicacion}\n\nAbre la app 'Archivos' de tu celular y busca la carpeta Descargas para verlo.")
    else:
        mostrar_popup_exito(f"✅ Archivo generado en:\n{ubicacion}")


def _generar_pdf_texto(titulo, lineas):
    """Genera un PDF simple (una o más páginas) con una lista de líneas de
    texto, sin depender de ninguna librería externa."""
    ancho, alto = 595, 842  # tamaño A4 en puntos
    margen_izq = 50
    y_inicio = alto - 60
    interlineado = 16
    lineas_por_pagina = max(1, int((y_inicio - 50) / interlineado))
    ...
    hijos = " ".join(f"{4 + i * 2} 0 R" for i in range(len(paginas)))
    objetos.append(f"<< /Type /Pages /Kids [{hijos}] /Count {len(paginas)} >>")
    ...
    for pagina in paginas:
        ...
        y = y_inicio
        for linea in pagina:
            contenido_lineas.append(f"BT /F1 11 Tf {margen_izq} {y} Td ({escapar(linea)}) Tj ET")
            y -= interlineado
        stream = "\n".join(contenido_lineas)
        objetos.append(f"<< /Type /Page /Parent 2 0 R /Resources << /Font << /F1 3 0 R >> >> /MediaBox [0 0 {ancho} {alto}] /Contents {len(objetos) + 2} 0 R >>")
...
def guardar_pdf_publico(filename, titulo, lineas):
    """Genera un PDF simple y lo guarda en Descargas."""
    contenido = _generar_pdf_texto(titulo, lineas)
    exito, ubicacion = _guardar_archivo_publico(filename, contenido, mime='application/pdf', encoding='latin-1')
    if exito:
        mostrar_popup_exito(f"✅ PDF guardado en:\n{ubicacion}\n\nAbre la app 'Archivos' de tu celular y busca la carpeta Descargas para verlo.")
    else:
        mostrar_popup_exito(f"✅ PDF generado en:\n{ubicacion}")
...
def crear_input(hint, password=False, multiline=False, text=None, input_filter=None):
    input_kwargs = {
        ...
        'cursor_color': get_color_from_hex(WHITE),
        'padding': dp(10),
        'password': password,
        'multiline': multiline,
        'size_hint_y': None,
...
class BotonRedondeado(Button):
    """Botón ovalado con sombra suave y efecto de presionado (más agradable a la vista)."""
    def __init__(self, rgba_color=(0.2, 0.4, 0.8, 1), **kwargs):
        ...
        with self.canvas.before:
            Color(0, 0, 0, 0.22)
            self.sombra = RoundedRectangle(pos=self.pos, size=self.size, radius=[dp(24)])
            self.color_instr = Color(*self.rgba_color)
            self.rect = RoundedRectangle(pos=self.pos, size=self.size, radius=[dp(24)])
        self.bind(pos=self._actualizar_rect, size=self._actualizar_rect, state=self._actualizar_estado)

    def _actualizar_rect(self, *args):
        radio = [min(self.height, dp(30)) / 2]
        self.rect.pos = self.pos
        self.rect.size = self.size
        self.rect.radius = radio
        self.sombra.pos = (self.x + dp(2), self.y - dp(2))
        self.sombra.size = self.size
        self.sombra.radius = radio

    def _actualizar_estado(self, *args):
        try:
            r, g, b, a = self.rgba_color
            if self.state == 'down':
                self.color_instr.rgba = (r * 0.75, g * 0.75, b * 0.75, a)
            else:
                self.color_instr.rgba = self.rgba_color
        except Exception:
            pass
...
class LoginScreen(Screen):
                ...
                for uid, user in data.items():
                    if user.get("nombre_usuario") == username:
                        ...
                        if hashed_input_password == user.get("contraseña"):
                            ...
                            chat_actual = app.data_cache.get("chat") or {}
                            _mensajes_notificados.update(chat_actual.keys())
                            Clock.unschedule(_revisar_mensajes_nuevos)
                            Clock.schedule_interval(_revisar_mensajes_nuevos, 20)
                            return
    ...
    def recuperar_contrasena(self, *args):
        ...
        content.add_widget(crear_label("Ingresa tu usuario o correo y la nueva contraseña que quieras usar", color=get_color_from_hex(GRAY)))
        ...
        def aviso_callback(success, error):
            pass  # el aviso por correo es informativo; no bloquea el cambio si falla
        ...
        def buscar_callback(success, data):
            ...
            if len(nueva) < 6:
                ...
                return
            if nueva != confirmar:
                mostrar_popup_error("Las contraseñas no coinciden")
                return
            ...
            for uid, user in data.items():
                if user.get("nombre_usuario") == valor or user.get("correo") == valor:
                    ...
                    def on_put(success2, response2):
                        if success2:
                            mostrar_popup_exito("✅ Tu contraseña fue actualizada.\n\nYa puedes iniciar sesión con tu nueva contraseña.")
                            if correo_usuario:
                                enviar_correo(
                                    destinatario=correo_usuario,
                                    asunto="Tu contraseña fue cambiada - RebañoEjecutivo",
                                    cuerpo="Hola,\n\nTu contraseña fue actualizada recientemente. Si no fuiste tú quien hizo este cambio, comunícate con el administrador.\n\n- RebañoEjecutivo",
                                    on_complete=aviso_callback
                                )
                        else:
                            mostrar_popup_error(f"❌ Error al actualizar la contraseña: {response2}")

                    firebase_put(f"usuarios/{uid}", user, on_complete=on_put)
                    return
            popup.dismiss()
        ...
        def buscar_trigger(*args):
            ...
            if not valor:
                ...
                return
            if not input_nueva.text.strip() or not input_confirmar.text.strip():
                mostrar_popup_error("Ingresa la nueva contraseña y confírmala")
                return
        ...
        content.add_widget(btn_cancel)
        popup = Popup(title='Recuperar Contraseña', content=content, background_color=get_color_from_hex(BLACK), size_hint=(0.9, 0.8))
...
class RegisterScreen(Screen):
    ...
    def registrar(self, *args):
        ...
        if datos['rol'] == 'director':
            def check_directores(success, data):
                if not success:
                    mostrar_popup_error(data)
                    return
                directores = [u for u in (data or {}).values() if u.get('rol') == 'director']
                if len(directores) >= 2:
                    mostrar_popup_error("Ya existen los 2 usuarios Director permitidos.\nNo se pueden registrar más con este rol.")
                    return
                continuar_registro()

            firebase_get("usuarios", on_complete=check_directores)
        else:
            continuar_registro()
...
class CalendarioScreen(Screen):
    ...
    def cargar_seguimientos(self):
        ...
        if not mis_seguimientos:
            ...
            btn_csv = crear_boton("📤 Exportar CSV", GREEN, lambda x, s=mis_seguimientos: self.exportar_seguimientos_csv(s), alpha=0.8)
            btn_pdf = crear_boton("📄 Exportar PDF", GREEN, lambda x, s=mis_seguimientos: self.exportar_seguimientos_pdf(s), alpha=0.8)
    ...
    def exportar_seguimientos_csv(self, seguimientos):
        filename = f"seguimientos_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.csv"
        headers = ["oveja", "fecha", "tipo", "contenido"]
        filas = [{
            "oveja": s.get("nombre_oveja", ""),
            "fecha": s.get("fecha_seguimiento", ""),
            "tipo": s.get("tipo", ""),
            "contenido": s.get("contenido", "")
        } for s in seguimientos]
        guardar_csv_publico(filename, headers, filas)

    def exportar_seguimientos_pdf(self, seguimientos):
        filename = f"seguimientos_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.pdf"
        lineas = []
        for s in seguimientos:
            lineas.append(f"{s.get('nombre_oveja', '')} - {s.get('fecha_seguimiento', '')} ({s.get('tipo', '')})")
            lineas.append(f"  {s.get('contenido', '')}")
            lineas.append("")
        guardar_pdf_publico(filename, "Seguimientos - RebañoEjecutivo", lineas)
...
class EstadisticasScreen(Screen):
    ...
    def _guardar_y_compartir_csv(self, filename, headers, filas):
        """Genera el CSV y lo guarda directamente en la carpeta de Descargas."""
        guardar_csv_publico(filename, headers, filas)
...
class RebañoApp(App):
    ...
    def on_stop(self):
        """Se ejecuta cuando el usuario sale/cierra la aplicación. Borra la
        sesión guardada para que la próxima vez pida iniciar sesión de nuevo."""
        try:
            if self.store and self.store.exists('user_session'):
