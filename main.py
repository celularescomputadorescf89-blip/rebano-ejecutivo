...
# ========================
def _guardar_archivo_publico(nombre_archivo, contenido_texto, mime='text/csv'):
    ...
    try:
        ...
        stream.write(contenido_texto.encode('utf-8'))
        stream.close()
...
class CampoRedondeado(TextInput):
    """Campo de texto con esquinas totalmente redondeadas (a juego con los botones)."""
    def __init__(self, rgba_color=(0.2, 0.2, 0.2, 1), **kwargs):
        super().__init__(**kwargs)
...
def crear_input(hint, password=False, multiline=False, text=None, input_filter=None):
    ...
    input_field = CampoRedondeado(rgba_color=(0.2, 0.2, 0.2, 1), **input_kwargs)
...
class EstadisticasScreen(Screen):
    ...
    def _guardar_y_compartir_csv(self, filename, headers, filas):
        """Genera el CSV en memoria y lo guarda directamente en la carpeta
        de Descargas del celular."""
        buffer = io.StringIO()
        ...
        exito, ubicacion = _guardar_archivo_publico(filename, contenido)
        if exito:
...
class ImportarOvejasScreen(Screen):
    def __init            mostrar_popup_error("Completa todos los campos")
            return
        if len(nueva) < 6:
            mostrar_popup_error("La nueva contraseña debe tener al menos 6 caracteres")
            return
...
class ImportarOvejasScreen(Screen):
    def __init# CONFIGURACIÓN DE FUENTE
# ========================
KIVY_FONT_FAMILY = None
